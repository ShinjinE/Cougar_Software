...
class NeckTiltOutput(OutputObject):
    """
    ...
    def get_output(self, input_name, input_value):
        ...
        if self.control_type is ControlType.DIRECT:
            ...
            raw_output = [0, 0]
            raw_output[0] = self.current_input[0] + self.current_input[1]
            raw_output[1] = self.current_input[0] - self.current_input[1] + self.maximum_input
            for i in range(self.num_outputs):
                self.current_output[i] = self.map_values(raw_output[i], self.out_raw_min, self.out_raw_max,
